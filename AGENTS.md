...
- **Re-exec the saved argv as a real argv, not a shell string.** `ps -o cmd= -p $FE_PID` reflows and masks the session key, so grab the raw argv with `tr '\0' '\n' < /proc/$FE_PID/cmdline > /tmp/fe-argv.txt` while the process is still alive. `--runtime-services-info` is a single JSON argument that contains spaces, so relaunching with `nohup $(tr '\n' ' ' < /tmp/fe-argv.txt)` word-splits it into many argv entries and node dies with `Unknown flag: <word>` (seen 2026-09-21). Re-exec the file instead, e.g. `setsid nohup python3 -c "import os; a=[l.rstrip() for l in open('/tmp/fe-argv.txt')]; os.execvp(a[0], a)" > /tmp/agent-canvas-static.log 2>&1 < /dev/null &`.
- `/opt/agent-canvas` may carry **uncommitted local edits** from a previous hot-deploy (e.g. a fix that was hand-applied to the running checkout before it was committed to `dev`). `git reset --hard origin/dev` silently discards them. Before resetting, `git fetch origin` (the checkout's `origin/dev` ref can be stale — it's only updated on fetch), then `git diff origin/dev -- <each modified file>` to confirm each local change is already present verbatim in the fetched `origin/dev`. Only if every diff is empty is the reset safe; otherwise the reset would drop a live, un-pushed fix and silently regress the running stack. If a local edit is *not* yet on `dev`, commit/push it first (or stash it across the reset) instead of clobbering it. (Encountered during the workspace-grouping deploy: `scripts/proxy-utils.mjs` had a local cookie-downgrade edit that turned out to already be on `dev` via a later merge — verified identical before resetting.)
...
- Mermaid diagrams in markdown (`src/components/features/markdown/`): a ```mermaid fence reaches every markdown surface (chat messages, plan preview, files-tab file preview) through the shared `code.tsx` `p` component, which reads the language off `className` as `language-<lang>` — Prism's whitelist has no `mermaid` entry, so before this change such fences fell through to a plain `<pre>` and rendered as source. `code.tsx` now dispatches `language-mermaid` to `MermaidBlock` (`mermaid.tsx`), whose `React.lazy(() => import("./mermaid-renderer"))` keeps the ~1 MB mermaid bundle out of the eagerly-loaded markdown graphs (Vite splits it per diagram type — `pieDiagram`, `sequenceDiagram`, `stateDiagram`, … — so only the types actually used download). The lazy component **must** be declared at module scope: creating `React.lazy` inside the component function trips `react-hooks/static-components` and would reset its own Suspense state on every render. `mermaid-renderer.tsx` calls `mermaid.render` in an effect, sanitizes the returned SVG with DOMPurify (`USE_PROFILES: {svg, svgFilters, mathMl}` + script/`foreignObject`/inline-handler blocklist) before `dangerouslySetInnerHTML`, and on a parse failure falls back to a `CopyableContentWrapper` showing the source plus `MERMAID$INVALID_DIAGRAM` instead of leaving an empty box. `mermaid.initialize` runs at module scope of the renderer (it is only reachable in the browser, inside the lazy chunk) with `startOnLoad: false` so mermaid never scans the DOM itself. **Testing trap:** mermaid's layout pass calls `getBBox` on SVG text, which jsdom does not implement, so `mermaid.render` rejects with `getBBox is not a function` for even a valid diagram — unit tests must `vi.mock("mermaid")` and drive the component's real promise/fallback state machine (`__tests__/components/features/markdown/mermaid-renderer.test.tsx`, `mermaid-code-dispatch.test.tsx`). Do not assert on arrow-containing source with `getByText`: React/DOMPurify text is HTML-escaped in the DOM (`->>` shows as `&gt;&gt;`), so compare `textContent` instead. The new key was also added to the identical-string allowlist in `scripts/check-translation-completeness.cjs` because only the English copy is meaningful for the diagnostic; `npm run make-i18n` regenerates `src/i18n/declaration.ts` + `public/locales/*` (both gitignored/untracked — generated artifacts are not committed). **Failure-path gotcha (mermaid 11):** `mermaid.render(id, chart)` appends a scratch `<div id="d{id}">` to `document.body` and removes it only on success — after a parse failure it stays behind holding the error diagram, which shows up as a stray `Syntax error in text / mermaid version <x>` box appended to the bottom of the page (the visible "mermaid broke the UI" symptom; nothing else in the app draws that text). `mermaid-renderer.tsx` therefore drops `#d{id}` in a `.finally()` on the render promise so both outcomes clean up. Mermaid has a second failure mode: it can *resolve* and still return its internal error diagram (`<g class="error">` carrying "Syntax error in text") instead of rejecting, so `isErrorDiagram()` scans the returned SVG for that marker and routes it to the same source fallback rather than inserting the error image as if it were a real diagram. Tests must not rely on the default 1000 ms `waitFor`: `MermaidRenderer` is `React.lazy`, and under the full parallel suite the dynamic import can outrun it — the test file passes a `LAZY_TIMEOUT` to every `waitFor`.
